              ...
              // }
                    steps {
                        ...
                        dir(path: env.BUILD_ID) {
                            ...
                            sh "docker run -v -privileged  '${env.WORKSPACE}/${env.BUILD_ID}/sources:/src'  --entrypoint pwd ${IMAGE} "
                            sh "docker run -v -privileged ${VOLUME} ${IMAGE} 'ls'"
                            sh "docker run -v -privileged '${env.WORKSPACE}/${env.BUILD_ID}/sources:/src' ${IMAGE} 'ls' "
                            sh "docker run -v -privileged ${env.WORKSPACE}/${env.BUILD_ID}/sources:/src ${IMAGE} 'ls' "
                            sh "docker run -v -privileged ${env.WORKSPACE}/${env.BUILD_ID}/sources  --entrypoint cat ${IMAGE} src/add2vals.py"
                            sh "docker run -v -privileged ${env.WORKSPACE}/${env.BUILD_ID}/sources  ${IMAGE} 'pyinstaller -F ${env.WORKSPACE}/${env.BUILD_ID}/sources/add2vals.py'"
